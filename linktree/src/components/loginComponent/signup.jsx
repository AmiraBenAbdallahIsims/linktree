...
import Navbar from "../homeComponent/navbarComponenet/navbar";
import Client from "./clientComponent/client";
import Designer from "./designerComponent/designer";
import Visitor from "./visitorComponent/visitor";
...
export default function Signup() {
    ...
    //     const token = 10;
    //     const res = await fetch('http://localhost:3001/api/testpostman', {
    //         method: 'GET',
    //         headers: {
    ...
    // useEffect(() => {
    //     // fetch('http://localhost:3001/api/testpostman')
        ...
        <div className="login">
            <div className="divkbira">
                <div className="divleft">
                    <svg className="logo" viewBox="0 0 1176 238" title="Linktree Logo" xmlns="http://www.w3.org/2000/svg" width="100" height="50">
                    ...
                    <div className="d-l-main-content">
                        <h1>Join Linktree</h1>
                        <p>Sign up for free as :</p>
                        <div className="butts">
                            <button className="butt" onClick={() => setFormType('client')}>client</button>
                            <button className="butt" onClick={() => setFormType('designer')}>designer</button>
                            <button className="butt" onClick={() => setFormType('visitor')}>visitor</button>
                        </div>
                        <div class="form-data">{renderForm()}</div>
                        <p className="already-acc-p">Already have an account? <span>Log in</span></p>
                        <p className="captcha-p">This site is protected by reCAPTCHA and the <span>Google Privacy Policy</span> and <span>Terms of Service</span> apply.</p>
                    </div>
                </div>
                <div className="divrigth"></div>
            </div>
        </div>
    );
}
